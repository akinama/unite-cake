...
"
function! unite#sources#cake#helper#gather_candidates_file(path)
  let files = []
  for d in split(globpath(unite#sources#cake#helper#cake_root(), a:path), '\n')
    if isdirectory(d)
...
"
function! unite#sources#cake#helper#gather_candidates_cmd(command, arguments)
...
"
function! unite#sources#cake#helper#gather_candidates_cmd_input(command, arguments) 
  return map(a:arguments , '{
        \ "word" : v:val.word ,
        \ "abbr" : get(v:val, "abbr", v:val.word) ,
        \ "kind" : "command" ,
...
function! s:create_cmd_input(cmd, word, abbr)
  return "call unite#sources#cake#helper#execute_cmd_input('". a:.cmd . "','" . a:word . "','" . a:abbr . "')"
endfunction
"
"
function! unite#sources#cake#helper#execute_cmd_input(cmd, word, abbr)
...
function! s:execute_cmd()
  if g:unite_cake_execute_cmd == '!'
    return '!'
  else
    return g:unite_cake_execute_cmd . ' '
  endif
endfunction
